...
var Puzzle = (function() {    
    ...
    Puzzle.makeColourable = function(node) {
        // Element starts off empty
        node.element.setAttribute('class', 'empty-region');

        // Clicking on the element changes its class
        // and updates its object
        node.element.addEventListener('click', function() {
            var colour = Puzzle.selectedColour;
            if (colour) {
                node.element.setAttribute('class', "colour-" + colour);
                node.colour = colour;
                Puzzle.evaluate();
            }
        });
